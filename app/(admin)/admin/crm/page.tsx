...
const COLUMNS = [
  { id: "TOUS_PROSPECTS", label: "Tous les prospects", color: "border-slate-600" },
    ...
    <div className="space-y-6 h-full flex flex-col">
      <div className="flex items-center justify-between flex-wrap gap-4 shrink-0">
        <div>
          ...
          <p className="text-slate-400 mt-1">{prospects.length} prospect{prospects.length > 1 ? "s" : ""}</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => router.push("/admin/crm/emails")}
            className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-slate-700 bg-slate-800/50 text-sm text-slate-300 hover:text-white transition-colors">
            <Mail className="h-4 w-4" /> Emails IA
          </button>
          <button className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-slate-700 bg-slate-800/50 text-sm text-slate-300 hover:text-white transition-colors">
      ...
      <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        <div className="flex-1 overflow-x-auto pb-4">
          <div className="flex gap-4" style={{ minWidth: COLUMNS.length * 280 }}>
            {COLUMNS.map((col) => {
              const colProspects = prospects.filter((p) => p.colonne === col.id);
              return (
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60">
          <div className="w-full max-w-md mx-4 rounded-xl border border-slate-700 bg-slate-900 p-6 space-y-4">
            <div className="flex items-center gap-3">
              <div className="grid h-10 w-10 place-content-center rounded-full bg-emerald-500/10">
                <CheckCircle2 className="h-5 w-5 text-emerald-400" />
              </div>
              <div>
                <h3 className="text-lg font-semibold text-white">Prospect gagné !</h3>
                <p className="text-sm text-slate-400">{gagnePopup.prospect.nom} est maintenant client</p>
              </div>
            </div>
            <p className="text-sm text-slate-300">Souhaitez-vous créer un chantier pour ce nouveau client ?</p>
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg bg-gradient-to-r from-amber-500 to-orange-600 text-white text-sm font-semibold transition-colors">
      ...
      {popup && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60">
          <div className="w-full max-w-lg mx-4 rounded-xl border border-slate-700 bg-slate-900 p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-white">
                {popup.action === "devis" ? "Aperçu du devis" : "Message de relance"}
              </h3>
              <button onClick={() => setPopup(null)} className="text-slate-500 hover:text-white"><X className="h-5 w-5" /></button>
            </div>
            <div className="text-sm text-slate-400">
              <span className="text-white font-medium">{popup.prospect.nom}</span> — {popup.prospect.email}
            </div>
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setPopup(null)} className="flex-1 py-2.5 rounded-lg border border-slate-700 text-sm text-slate-400 hover:text-white transition-colors">Annuler</button>
              <button onClick={sendEmail} disabled={sendingEmail}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-slate-900 font-semibold text-sm transition-colors">
...
function DroppableColumn({ col, count, children }: { col: typeof COLUMNS[0]; count: number; children: React.ReactNode }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`w-72 shrink-0 rounded-xl border bg-slate-900/50 p-3 transition-colors ${
    ...
    >
      <div className={`flex items-center gap-2 mb-3 pb-2 border-b ${col.color}`}>
        {Icon && <Icon className={`h-4 w-4 ${col.id === "GAGNE" ? "text-emerald-400" : "text-red-400"}`} />}
        <h3 className="text-sm font-semibold text-white flex-1">{col.label}</h3>
        <span className="text-xs bg-slate-800 text-slate-400 rounded-full px-2 py-0.5">{count}</span>
      </div>
      {children}
    </div>
  );
...
function ProspectCardUI({ prospect, isDragging }: { prospect: Prospect; isDragging?: boolean }) {
  ...
  return (
    <div className={`rounded-lg border border-slate-700/50 bg-slate-800/50 p-3 ${isDragging ? "shadow-2xl opacity-90 rotate-2 scale-105" : "hover:border-slate-600"} transition-all cursor-grab active:cursor-grabbing`}>
      <div className="flex items-start gap-3">
        <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0" style={{ background: `hsl(${hue}, 60%, 40%)` }}>
          {initials}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-white truncate">{prospect.nom}</p>
          <p className="text-xs text-slate-500 flex items-center gap-1 mt-0.5">
            <HardHat className="h-3 w-3" /> {prospect.typeChantier}
          </p>
        </div>
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60">
      <div className="w-full max-w-md mx-4 rounded-xl border border-slate-700 bg-slate-900 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-white">Nouveau prospect</h3>
