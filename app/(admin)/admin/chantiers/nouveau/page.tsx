    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400">Renseignez les informations du chantier</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {selectedClient ? (
            ...
            <div className="flex gap-3">
              ...
              <button type="button" onClick={() => setShowNewClientModal(true)}
                className="flex items-center gap-2 rounded-lg border border-amber-500/30 px-4 py-2.5 text-sm font-medium text-amber-400 hover:bg-amber-500/10 transition-colors whitespace-nowrap">
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900 p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1.5">Type de chantier</label>
              <select
                value={form.type}
                onChange={(e) => set("type", e.target.value)}
                className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-amber-500/50"
              >
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Notes / observations</label>
            <textarea
              rows={3}
              value={form.description}
              onChange={(e) => set("description", e.target.value)}
              className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500/50 resize-none"
        ...
        {/* Photos */}
        <div className="rounded-xl border border-slate-800 bg-slate-900 p-6 space-y-4">
          <h2 className="text-base font-semibold text-white">Photos du chantier</h2>
          <div className="flex flex-wrap gap-3">
            {photos.map((photo, i) => (
              <div key={i} className="relative group">
                <div className="w-24 h-24 rounded-lg bg-slate-800 border border-slate-700 overflow-hidden">
            ...
            <label className="w-24 h-24 rounded-lg border-2 border-dashed border-slate-700 flex flex-col items-center justify-center cursor-pointer hover:border-amber-500/50 transition-colors">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-600 disabled:bg-slate-700 disabled:text-slate-500 text-slate-900 font-semibold py-3 rounded-lg transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 rounded-lg border border-amber-500/30 text-amber-400 hover:bg-amber-500/10 font-medium text-sm transition-colors"
          >
      ...
      {showNewClientModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setShowNewClientModal(false)}>
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold text-white">Nouveau client</h2>
              <button onClick={() => setShowNewClientModal(false)} className="text-slate-400 hover:text-white"><X className="h-5 w-5" /></button>
            </div>
            ...
            <div className="mt-5 flex justify-end gap-3">
              <button onClick={() => setShowNewClientModal(false)} className="rounded-lg border border-slate-700 px-4 py-2 text-sm text-slate-300 hover:bg-slate-800">Annuler</button>
              <button onClick={handleCreateClient} disabled={!newClient.nom.trim() || savingClient}
                className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-amber-500 to-orange-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">
                {savingClient && <Loader2 className="h-4 w-4 animate-spin" />} Créer
              </button>
            </div>
          </div>
        </div>
